...
# List of proxies (Webshare proxies)
PROXIES = [
...
def make_request_with_proxy(url, method='POST', headers=None, data=None, cookies=None, max_retries=3):
    ...
    for attempt in range(max_retries):
        try:
            ...
            print(f"Attempt {attempt + 1}: Using proxy {proxy.split('@')[-1]}")
...
def stripe_payment(site, card_data):
    try:
        ...
        # Make first request to Stripe using proxy
        print(f"Making Stripe request for site: {site}")
        ...
        # Make second request to merchant site using proxy
        print(f"Making merchant site request for site: {site}")
...
def proxies_info():
    ...
    return jsonify({
        ...
        'proxies': [proxy.split('@')[-1] for proxy in PROXIES]
    })
